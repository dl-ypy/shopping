...
<body>
    ...
    <div class="formtext">Hi，<b>admin</b>，欢迎您使用修改商品类别功能！</div>
    
    <form action="back/McTypeServlet?task=update" method="post" onsubmit="return ruleNumKeyUp()">
	    <ul class="forminfo">
	    <li><input type="hidden" name="typeid" value="${mt.typeid}"></li>
	    <li><label>类别名称<b>*</b></label><input id="name" name="typename" type="text" 
	    	class="dfinput" value="${mt.typename}" onkeyup="ruleNumKeyUp()"/><i id="remind">标题不能超过30个字符</i>
	    </li>
	    	...
	    	<div class="vocation">
			    <select class="select1" name="fatherid">
			    	...
			    	<c:forEach var="list" items="${list}">
				    	<option value="${list.typeid}" ${list.typeid eq mt.fatherid?'selected':''}>${list.typename}</option>
			   		</c:forEach>
			    </select>
		    </div>
	    </li>
